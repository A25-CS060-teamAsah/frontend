  ...
  const fetchPendingLeads = async () => {
    try {
      ...
      const pending = response.customers.filter(
        (customer) => customer.probability_score === null || customer.probability_score === undefined
      );

      setPendingLeads(pending);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load pending leads");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          ...
          <p className="text-xs text-gray-500 mt-2">Customers without predictions</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="px-6 py-5 border-b border-gray-100">
          <h3 className="text-lg font-bold text-gray-800">Pending Predictions</h3>
          <p className="text-sm text-gray-500 mt-1">Customers awaiting ML prediction scores</p>
        </div>

        {totalPending === 0 ? (
          <div className="px-6 py-12 text-center">
            <Clock className="w-16 h-16 text-gray-300 mx-auto mb-4" />
            <h4 className="text-lg font-semibold text-gray-700 mb-2">No Pending Predictions</h4>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Education</th>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">{lead.marital}</p>
                          </div>
                        </div>
                      </td>
                    <td className="px-6 py-4">
                      <p className="text-sm text-gray-800">{lead.age} years</p>
                    </td>
                    <td className="px-6 py-4">
                      <p className="text-sm text-gray-800">{lead.job}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-600">{lead.education}</span>
                    </td>
                    <td className="px-6 py-4">
